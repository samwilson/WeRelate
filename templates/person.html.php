<div class="infobox">
    ...
    <h1 class="title"><?php echo $person->getFullName() ?></h1>
	<ul class="bmd">
		<li>
...
<?php foreach ( $person->getFamilies( 'spouse' ) as $family ): ?>
	...
	<ul>
	<li>H. <?php if ( $h=$family->getSpouse( 'husband' ) ) { echo Linker::link( $h->getTitle(), $h->getFullName() );
					   } ?></li>
	<li>W. <?php if ( $w=$family->getSpouse( 'wife' ) ) { echo Linker::link( $w->getTitle(), $w->getFullName() );
					   } ?></li>
	<ol>
...
foreach ( $fact['sources'] as $source_id ) {
	if ( $source = $person->getSource( $source_id ) ) {
		...
		echo $parser->recursiveTagParse( '<ref name="'.$source_id.'">'.$title.' '.$source['body'].'</ref>' );
	}
} ?></th>
